...
pub enum TaskIssueProviderConfig {
    GitHub(u16, Option<String>),
    GitLab(u16, Option<String>),
    O365(u16, Option<String>),
}

impl fmt::Display for TaskIssueProviderConfig {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            TaskIssueProviderConfig::GitHub(todosrc_idx, x) => {
                write!(f, "{}{}/{:?}", SHORT_CODE_GITLAB, todosrc_idx, x)
            }
            TaskIssueProviderConfig::GitLab(todosrc_idx, x) => {
                write!(f, "{}{}/{:?}", SHORT_CODE_GITHUB, todosrc_idx, x)
            }
            TaskIssueProviderConfig::O365(todosrc_idx, x) => {
...
impl std::str::FromStr for TaskIssueProviderConfig {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if let Some(captures) = TASK_ID_RE.captures(s) {
            ...
            match task_source {
                SHORT_CODE_GITHUB => Ok(TaskIssueProviderConfig::GitHub(todosrc_idx, issue_id)),
                SHORT_CODE_GITLAB => Ok(TaskIssueProviderConfig::GitLab(todosrc_idx, issue_id)),
                SHORT_CODE_O365 => Ok(TaskIssueProviderConfig::O365(todosrc_idx, issue_id)),
...
impl TaskIssueProviderConfig {
    ...
    pub fn task_supplier<'a>(self, app_config: &'a AppConfig) -> TaskIssueProvider {
        match self {
            TaskIssueProviderConfig::GitHub(todosrc_idx, _) => {
                ...
                TaskIssueProvider::GitHub(github_repo.expect("Invalud github index").clone())
            }
            TaskIssueProviderConfig::GitLab(todosrc_idx, _) => {
                let gitlab_config = &app_config.gitlab_com;
                let gitlab_repo = gitlab_config
                    .as_ref()
                    .and_then(|config| config.repositories.get(todosrc_idx as usize));
                TaskIssueProvider::GitLab(gitlab_repo.expect("Invalud gitlab index").clone())
            }
            TaskIssueProviderConfig::O365(todosrc_idx, _) => {
                let o365_config = &app_config.o365;
                let o365_repo = o365_config
                    .as_ref()
                    .and_then(|config| config.todo_lists.get(todosrc_idx as usize));
                TaskIssueProvider::O365(o365_repo.expect("Invalud o365 index").clone())
            }
        }
    }
}
